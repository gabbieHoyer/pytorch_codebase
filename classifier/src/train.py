...
def train(model, dataloaders, args):
    ...
    if args.decay_steps > 0:
        ...
        lr_decay = lr_scheduler.ReduceLROnPlateau(optimiser, factor=args.decay_rate, mode='max',
                                                  patience=60, cooldown=20, min_lr=1e-6, verbose=True)
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (inputs, labels) in enumerate(train_dataloader):
            # continue
            args.step = (epoch * len(dataloaders['train'])) + i  # calc current step
            ...
            bias = torch.argmax(yhat, dim=1).float().sum() / args.batch_size
            true_bias =  labels.float().sum() / args.batch_size
            ...
            if args.print_progress:
                ...
                args.writer.add_scalar('Loss/train', loss, args.step)
                args.writer.add_scalar('Pred/bias', bias, args.step)
                args.writer.add_scalar('Pred/true_bias', true_bias, args.step)
        ...
        if args.print_progress:  # only validate using process 0
            ...
            logging.info(f'\n[Train] loss: {epoch_train_loss:.4f} - acc: {epoch_train_acc:.4f} | [Valid] loss: {epoch_valid_loss:.4f} - acc: {epoch_valid_acc[0]:.4f} ')
