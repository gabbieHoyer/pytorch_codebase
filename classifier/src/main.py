...
PARSER.add_argument('--dataset', default='cifar10',
                    help='e.g. bone_aging, cifar10, fashionmnist, patch_anomaly_detection')

PARSER.add_argument('--n_epochs', type=int, default=100,
                    help='number of epochs to train for.')

PARSER.add_argument('--n_chans', type=int, default=3,
                    help='number of epochs to train for.')
...
PARSER.add_argument('--visible_gpu', dest = 'visible_gpus',
                    help='Define which GPUs are visible.')
try:
    PARSER.set_defaults(visible_gpus=os.environ['NV_GPU'])
except:
...
PARSER.add_argument('--use_net', default='resnet',
...
def main():
    ...
    #model = models4finetuning.initialize_model(model_name = args.use_net, num_classes=args.n_classes, use_as_feature_extractor=args.as_feature_extractor, use_pretrained=args.use_pretrained)
    ...
    if not args.inference:
        ...
        if args.extract_representation == True:
            test_loss, test_acc, all_intermediate_features, all_labels, all_predictions, all_output_activations = evaluate(model, dataloaders['test'], args)
        ...
        logging.info(f'[Test] loss {test_loss:.4f} - acc: {test_acc[0]:.4f}')
